...
exports.createUserToken = async (data) => {
  const createdAt = moment().format('YYYY-MM-DD HH:mm:ss');
  const query = db.write('user_token').insert({
    user_id : data.user_id,
    token: data.token,
    expiry: data.expiry,
    created_at: createdAt,
...
exports.updateUserToken = async (data) => {
  const query = db.write('user_token')
...
exports.getUserTokenByUserId = async (user_id) => {
  const query = db.read.select('user_token.token', 'user_token.expiry')
  .from('user_token')
  .where('user_id', '=', user_id)
  console.info("query -->", query.toQuery())
  return query;
...
exports.createEmailToken = async (data) => {
  const createdAt = moment().format('YYYY-MM-DD HH:mm:ss');
  const query = db.write('auth').insert({
    user_id : data.user_id,
    email : data.email,
    token : data.token,
